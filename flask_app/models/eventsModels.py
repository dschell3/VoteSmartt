...
class Events:
    def __init__(self, data):
        ...
        def createEvent(cls, data):
            query = '''
            INSERT INTO events (title, description, start_time, end_time, created_by, created_at)
            VALUES (%(title)s, %(description)s, %(start_time)s, %(end_time)s, %(created_by)s, NOW());
            '''
        ...
        def editEvent(cls, data):
            query = '''
            UPDATE events SET title = %(title)s,
